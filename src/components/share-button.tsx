...
function getShareTitle(title: string, latitude: number, longitude: number): string {
  if (title.trim()) return title;
  return `GeoChat conversation near ${latitude.toFixed(4)}, ${longitude.toFixed(4)}`;
}
...
export default function ShareButton({
  ...
  const shareTitle = getShareTitle(title, latitude, longitude);
  const shareText = `A conversation on GeoChat at ${latitude.toFixed(4)}, ${longitude.toFixed(4)}`;
  ...
  const handleEmail = useCallback(() => {
    setDropdownOpen(false);
    const subject = encodeURIComponent(`${shareTitle} — GeoChat`);
    const body = encodeURIComponent(`Check out this conversation on GeoChat:\n\n${shareUrl}`);
